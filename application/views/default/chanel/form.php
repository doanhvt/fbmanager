    ...
    <form class="form-horizontal e_ajax_submit" action="<?php echo $save_link; ?>" enctype="multipart/form-data" method="POST">
        ...
        <div class="modal-body bgwhite">
            <?php foreach ($list_input as $input) { ?>
                <div class="control-group <?php echo (preg_match("/type\=hidden/i", $input->rule)) ? "hidden" : ""; ?>">
                    ...
                    <div class="controls controls-row">
                        ...
                        <?php } else { ?>
                            <input <?php if(isset($edit_status) &&  $input->name == 'code'){ echo 'disabled style="background: #d9d9d9"';} ?> name="<?php echo $input->name; ?>" <?php echo $input->rule; ?> id="i_<?php echo $input->name; ?>" />
                        <?php } ?>
                    </div>
                </div>
            <?php } ?>
            ...
            if($perId < 3){ ?>
                ...
                <div class="controls controls-row">
                    <select class="select2" multiple="" name="list_id[]">
                        <option value="0">- Select -</option>
                        <?php if(isset($list_user)){
                            foreach ($list_user as $item) {
                                if(isset($edit_status)){
                                    if(isset($list_assign_chanel[$item->userId])){ ?>
                                        <option selected="" value="<?php echo $item->userId ?>"><?php echo $item->userDisplayName; ?></option>
                                    <?php }  else { ?>
                                        <option value="<?php echo $item->userId ?>"><?php echo $item->userDisplayName; ?></option>
                                    <?php }
                                } else { ?>
                                    <option value="<?php echo $item->userId ?>"><?php echo $item->userDisplayName; ?></option>
                                <?php }?>
