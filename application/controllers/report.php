 ...
 * @author Loc
 */
class report extends home_base {
    ...
    public function view_click_chanel() {
        ...
        $data['link_change_camp'] = site_url("report/do_change_campaign");
        ...
        if ($id_campaign) {
            ...
            $data["list_chanel"] = $this->m_campain_assign_chanel->get_list(array('m.campaign_id' => $id_campaign));
        } else {
        ...
        // Phan hien thi lai dau len, xuong cua sap xep
        $order_class = array(
            'number_c2' => 'sorting',
            'number_c3' => 'sorting',
            'c3perc2' => 'sorting'
        ...
        if ($order) {
            $order_class[$order] = ($order_type == 1) ? 'sorting_desc' : 'sorting_asc';
        }
...
//        var_dump($data['records']);exit;
            echo $this->load->view($this->path_theme_view . "report/view_click_chanel_ajax", $data, true);
            exit;
        }
    ...
    public function view_click_hour() {
        ...
        if ($this->input->is_ajax_request()) {
            echo $this->load->view($this->path_theme_view . "report/view_click_hour_ajax", $data, true);
            ;
            exit;
        }
        ...
        $header_page = $this->load->view($this->path_theme_view . "report/header", $data, true); /* Dữ liệu đẩy thêm vào thẻ <head> (css, js, meta property) */
        $title = 'Statistics of Click and Submition';
        $description = NULL;
        $keywords = NULL;
        $canonical = NULL;
    ...
    public function marketing_productivity() {
        ...
        if ($this->input->is_ajax_request()) {
            echo $this->load->view($this->path_theme_view . "report/marketing_productivity_ajax", $data, true);
            ;
            exit;
        }
        ...
        $header_page = $this->load->view($this->path_theme_view . "report/header", $data, true); /* Dữ liệu đẩy thêm vào thẻ <head> (css, js, meta property) */
        $title = 'Productivity of Marketing';
        $description = NULL;
        $keywords = NULL;
        $canonical = NULL;
    ...
    public function get_data_view_click_chanel() {
        ...
        if ($code_chanel) {
            ...
            if ($id_campaign) {
                $this->load->model("m_campain_assign_chanel");
                $list_chanel_parent = $this->m_campain_assign_chanel->get_list(array('m.campaign_id' => $id_campaign));
            } else {
        ...
        foreach ($list_chanel_parent as $item) {
            if ($item->id != -100) {
                ...
                $array_info_ads = new stdClass();
                $array_info_ads->name_chanel = '';
                $array_info_ads->number_c2 = 0;
                $array_info_ads->number_c3 = 0;
                $array_info_ads->c3perc2 = 0;
                $item->list_ads['_sum_'] = $array_info_ads;
                $data[$item->id] = $item;
            } else {
                $item->stt = ++$stt;
                $item->list_ads = array();
                $item->name_chanel = $item->name;
                $item->number_c2 = 0;
                $item->number_c3 = 0;
                $item->c3perc2 = 0;
                $data[$item->id] = $item;
        ...
        foreach ($records_click as $item) {
            if ($item->parent_id == -100 && isset($data[$item->parent_id])) {
                ...
                if (isset($data[$item->parent_id]->list_ads[$item->id_ads])) {
                    ...
                    $data[$item->parent_id]->list_ads['_sum_']->number_c2 += 1;
                    $total_click += 1;
                } else {
                    $total_click += 1;
                }
        ...
        foreach ($records_submit as $item) {
            if ($item->parent_id == -100 && isset($data[$item->parent_id])) {
                ...
                if (isset($data[$item->parent_id]->list_ads[$item->id_ads])) {
                    ...
                    $data[$item->parent_id]->list_ads['_sum_']->number_c3 += 1;
                    ...
                    $data[$item->parent_id]->list_ads['_sum_']->c3perc2 = $data[$item->parent_id]->list_ads['_sum_']->number_c2 ? number_format(($data[$item->parent_id]->list_ads['_sum_']->number_c3 * 100) / $data[$item->parent_id]->list_ads['_sum_']->number_c2, "1", ".", "") : 0;
                    $total_submit += 1;
                } else {
                    $total_submit += 1;
                }
        ...
        /**
         * Lọc bỏ các kênh quảng cáo có số c2,c3 bằng 0
         */
        foreach ($data as $key_chanel => $item_record) {

            foreach ($item_record->list_ads as $key => $item) {
                if((!$item->number_c2 && !$item->number_c3) && $key_chanel != 2508){
                    unset($data[$key_chanel]->list_ads[$key]);
                }
            }
        }
        ...
        if ($order) {
            ...
            if (is_numeric($order_type)) {
                foreach ($data as $key => $item) {
                    if (isset($data[$key]->list_ads['_sum_'])) {
                        $arr_sum = $data[$key]->list_ads['_sum_'];
                        unset($data[$key]->list_ads['_sum_']);
                    }
...
//        $code_mauqc = $this->session->userdata('code_mauqc');
        ...
        if ($id_campaign) {
            $this->load->model("m_campain_assign_chanel");
            $list_chanel = $this->m_campain_assign_chanel->get_list(array('m.campaign_id' => $id_campaign));
        } else {
            $this->load->model("m_chanel");
        ...
        foreach ($records_click as $item) {
            ...
            if (isset($data[$h])) {
                if (isset($data[$h]->list_chanel[$item->parent_id])) {
                    ...
                    $data[$h]->list_chanel['_sum_']['total_click'] += 1;
                } else {
                    $data[$h]->list_chanel['_sum_']['total_click'] += 1;
                }
            }
            ...
            if (isset($data[$h])) {
                if ($item->parent_id == -100) {
                    if (isset($data[$h]->list_chanel[$item->parent_id])) {
                        ...
                        $data[$h]->list_chanel['_sum_']['total_submit'] += 1;
                    }
                } else {
                    if (isset($data[$h]->list_chanel[$item->parent_id])) {
                        $data[$h]->list_chanel[$item->parent_id]['total_submit'] += 1;
        ...
        foreach ($data as $date => $item) {
            ...
            foreach ($item->list_chanel as $key => $value) {
                if ($key != "Total") {
                    ...
                    // + vao cost/ngay
                    $data[$date]->list_chanel["_sum_"]["cost"] += intval($data[$date]->list_chanel[$key]["cost"]);
                    $data[$date]->list_chanel["_sum_"]["total_c1"] += intval($data[$date]->list_chanel[$key]["total_c1"]);
                }
                ...
                if ($total_submit == 0) {
                    ...
                    $data[$date]->list_chanel[$key]["cost_c3"] = round($data[$date]->list_chanel[$key]["cost"] / $total_submit);
                }
                // range
...
//        $total_cost_c3_sys = 0;
        ...
        foreach ($data as $key => $item_record) {
            ...
            if ($code_chanel) {
                ...
                if ($item_record->list_chanel[$code_chanel]['total_submit'] == 0) {
                    ...
                    $data_return[$key]->list_chanel[$code_chanel]['c3/total_c3'] = number_format(($item_record->list_chanel[$code_chanel]['total_submit'] * 100) / $item_record->list_chanel['_sum_']['total_submit'], "2", ".", "") . "%";
                }
                ...
                $total_cost += $item_record->list_chanel[$code_chanel]['cost'];
                $total_c1_sys += $item_record->list_chanel[$code_chanel]['total_c1'];
                $total_c2_sys += $item_record->list_chanel[$code_chanel]['total_click'];
                $total_c3_sys += $item_record->list_chanel[$code_chanel]['total_submit'];
                ...
                foreach ($item_record->list_chanel as $chanel => $item) {
                    ...
                    if ($chanel != '_sum_') {
                        if ($item_record->list_chanel['_sum_']['total_submit'] == 0) {
                            $data_return[$key]->list_chanel[$chanel]['c3/total_c3'] = '0%';
                        } else {
                            $data_return[$key]->list_chanel[$chanel]['c3/total_c3'] = number_format(($item['total_submit'] * 100) / $item_record->list_chanel['_sum_']['total_submit'], "2", ".", "") . "%";
                        }
                    } else {
        ...
        $data_return['total_sys']->cost_c2 = $total_c2_sys != 0 ? number_format(round($total_cost / $total_c2_sys)) : 0;
        $data_return['total_sys']->c2_c1 = number_format($total_c2_c1_sys, "2", ".", "") . '%';
        $data_return['total_sys']->total_c3 = number_format($total_c3_sys);
        $data_return['total_sys']->cost_c3 = $total_c3_sys != 0 ? number_format(round($total_cost / $total_c3_sys)) : 0;
    ...
    public function get_chart_view_click_hour() {
        $data = $this->session->userdata('data_view_hour');
...
//            }
    ...
    public function do_change_campaign() {
        $campaign_id = $this->input->post('id');
        $list_assign_chanel = array();
        if ($campaign_id) {
            $this->load->model("m_campain_assign_chanel");

            $list_chanel = $this->m_campain_assign_chanel->get_list(array('m.campaign_id' => $campaign_id));
            foreach ($list_chanel as $value) {
                $list_assign_chanel[$value->chanel_id] = $value;
            }
        } else {
            $this->load->model("m_chanel");
            $list_chanel = $this->m_chanel->get_list_chanel_parent();
            foreach ($list_chanel as $value) {
                $list_assign_chanel[$value->id] = $value;
            }
        }

        $data_return = array(
            'status' => '1',
            'list_assign_chanel' => $list_assign_chanel,
        );
