 ...
 * @author Loc
 */
class contact_new extends manager_base {
    ...
    public function view_export_contact() {
        $data = array();
        $data["ajax_link"] = site_url($this->name["class"] . "/export_contact");
        $data['total'] = $this->data->get_list_contact_handover();
        $data['total_mol'] = $this->data->get_list_contact_handover(true, array('m.status_mol' => 1));
        $data['total_crm'] = $this->data->get_list_contact_handover(true, array('m.status' => 1, 'm.status_mol' => 1));
        $data['total_crm_handover'] = $this->data->get_list_contact_handover(true, array('m.status' => 1, 'm.status_mol' => 1, 'm.status_handover' => 1));
        $data['total_crm_not_handover'] = $this->data->get_list_contact_handover(true, array('m.status' => 1, 'm.status_mol' => 1, 'm.status_handover' => 2));
        $data["title"] = $title = 'Contact từ ngày 07/11/2014';

        $viewFile = $this->name["view"] . '/' . 'form_contact_handover';
    ...
    public function export_contact() {
        ...
        $this->load->model('m_contact_new');
        $this->load->model('m_contact_handover');
        $data = $this->data->get_list_contact_handover(FALSE, array('m.status' => 1, 'm.status_mol' => 1, 'm.status_handover' => 2), $data_post['limit_contact']);

        if ($data && count($data)) {
            foreach ($data as $item) {
                $this->m_contact_new->update($item->id, array('status_handover' => 1));
                $user_id = $this->session->userdata('id');
                $this->m_contact_handover->add(array('contact_id' => $item->id, 'user_id' => $user_id, 'time' => time()));
            }
        }
        ...
        //Fill color header
        $sheet->getActiveSheet()->getStyle('A6:P6')->applyFromArray(
                array(
                    'fill' => array(
        ...
        $sheet->getActiveSheet()->getStyle('A6:P6')->getFont()->setBold(true)
                ->setName('Verdana')
                ->setSize(10)
    ...
    public function export_contact_old() {
        $json_conds = $this->session->userdata("contact_new_conditions");
        $array_conds = json_decode($json_conds, true);

        $this->load->model('m_contact');
        $this->m_contact->custom_conds = $array_conds;
        $data = $this->m_contact->get_list_contact_handover();
//         Loc contact trung SDT
    ...
    function get_search_condition($data = array()) {
        ...
        $condition_list = array(
            'name', "keyword", "parent_id", "code_mauqc", "code_campaign", "code_landingpage", "from_date", "to_date", 'link_cv', 'status'
        );
        $where_data = array();
        $like_data = array();
        $status = 0;
                ...
                if (isset($data[$cond])) {
                    switch ($cond) {
                        ...
                        case 'status':
                            if (trim($data['status']) != "") {
                                if ($data['status'] == 1) {
                                    $where_data['m.status_mol'] = 1;
                                    $status = 1;
                                }
                                if ($data['status'] == 2) {
                                    $where_data['m.status_mol'] = 1;
                                    $where_data['m.status'] = 1;
                                    $status = 2;
                                }
                                if ($data['status'] == 3) {
                                    $where_data['m.status'] = 2;
                                    $status = 3;
                                }
                            }
                            break;
                        case 'link_cv':
                            if (trim($data['link_cv']) != "") {
                                if ($data['link_cv'] == 1) {
                                    $where_data['m.link_cv'] = '';
                                }
                                if ($data['link_cv'] == 2) {
                                    $where_data['m.link_cv !='] = '';
                                }
        ...
        $this->session->set_userdata("contact_new_conditions", json_encode($data_return));
        return $data_return;
    }
    ...
    public function ajax_list_data($data = Array()) {
        ...
        $data["form_url"] = site_url("contact_new/ajax_list_data");
        $this->load->model("m_chanel");
        $this->load->model("m_campaign");
        $this->load->model("m_landing_page");
        $data["total_c3"] = count($this->data->get_contact_status(array()));
        $data["total_c3_1"] = count($this->data->get_contact_status(array('m.status_mol' => 1)));
        $data["total_l1"] = count($this->data->get_contact_status(array('m.status_mol' => 1, 'm.status' => 1)));
    ...
    public function manager($data = Array()) {
        ...
        $data['export_contact'] = site_url("contact_new/view_export_contact");
        $data['export_contact_old'] = site_url("contact_new/export_contact_old");
        $data['check_crm'] = site_url("contact_new/check_crm");
        parent::manager($data);
    }
    ...
    public function check_crm() {
        $this->load->model('m_contact_new');
        $data['list_contact'] = $this->m_contact_new->get_list(array('m.status' => 3));
        $data["ajax_link"] = site_url($this->name["class"] . "/do_check_crm");
        $data["title"] = $title = "Danh sách tài khoản chưa được kiểm tra với kho CRM";

        $viewFile = $this->name["view"] . '/' . 'form_check_crm';
        ...
        $this->master_page($content, $head_page, $data["title"]);
    }

    public function do_check_crm() {
        $this->load->model('m_contact_new');
        $list_contact = $this->m_contact_new->get_list(array('m.status' => 3), 5);
        if ($list_contact && count($list_contact)) {
            foreach ($list_contact as $item) {
                ...
                if ($arr_check_crm) {
                    if (isset($arr_check_crm->status) && ($arr_check_crm->status == 'True')) {
                        $this->m_contact_new->update($item->id, array(
                            'status' => 2
                        ));
                    } else {
                        $this->m_contact_new->update($item->id, array(
        ...
        $data_return['state'] = 1;
        $data_return['redirect'] = site_url($this->name["class"] . "/manager");
        $data_return['msg'] = 'Check lại với CRM thành công';
        echo json_encode($data_return);
        return TRUE;
    }
