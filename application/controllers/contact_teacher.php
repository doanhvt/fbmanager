...
class contact_teacher extends manager_base {
    ...
    public function setting_class() {
        $this->name = Array(
            ...
            "view" => "contact_teacher",
            "model" => "m_contact_teacher",
            "object" => "Contact nhân sự"
        );
    }
    ...
    public function _process_data_table($record) {
        ...
        if (is_array($record)) {
            ...
            if (isset($record->link_cv) && $record->link_cv) {
                if(strpos($record->link_cv,"http:/") === FALSE){
                    $record->link_cv = 'http://' . $record->link_cv;
                }
                $record->name = '<a target="_blank" style="color:#810c15" href="' . $record->link_cv . '">' . $record->name . '</a>';
                $record->link_cv = '<a target="_blank" style="color:#810c15" href="' . $record->link_cv . '">Download</a>';
            }
        }
        return $record;
    }
    ...
    public function export_contact() {
        ...
        foreach ($data as $record_item) {
            ...
            if ($record_item->link_cv) {
                if(strpos($record_item->link_cv, 'http://') === FALSE){
                    ...
                    $data_row->link_cv = $record_item->link_cv;
                }
            } else {
                $data_row->link_cv = '';
            }
            ...
            $data_row->language = $record_item->language;
        ...
        //Fill color header
        $sheet->getActiveSheet()->getStyle('A6:R6')->applyFromArray(
                array(
                    'fill' => array(
        ...
        $sheet->getActiveSheet()->getStyle('A6:R6')->getFont()->setBold(true)
                ->setName('Verdana')
                ->setSize(10)
    ...
    public function ajax_list_data($data = Array()) {
        ...
        $data["form_url"] = site_url("contact_teacher/ajax_list_data");
        $this->load->model("m_chanel");
        $this->load->model("m_campaign");
        $this->load->model("m_landing_page");
    ...
    public function manager($data = Array()) {
        ...
        $data['export_contact'] = site_url("contact_teacher/export_contact");
        parent::manager($data);
    }

    protected function get_left_content($data = Array()) {
        $data["menu_data"] = $this->_get_left_content_data();
        return $this->load->view($this->site_config->path_theme_view . "contact_teacher/left_content", $data, TRUE);
    }
    ...
    protected function master_page($content, $head_page = NULL, $title = NULL, $description = NULL, $keywords = NULL, $canonical = NULL) {
        ...
        $this->load->view($this->site_config->path_theme_view . "contact_teacher/master_page", $data);
    }
